...
export const CreateAddonForm = ({ onCloseAction, onSubmitAction }: CreateAddonFormProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const invalidItems = formData.applicableItems.filter((item) => !item.name.trim() || !item.price.trim())
    ...
    try {
      const response = await fetchWithAuth("http://localhost:11000/v1/manager/create-addons", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || "Failed to create addon")
      }

      const data = await response.json()
      toast.success("Addon created successfully")

      // Pass the created addon back to the parent component and close the modal
      onSubmitAction(data.addon || formattedData)
      onCloseAction() // Make sure to call this to close the modal
    } catch (error) {
